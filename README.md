...
목표는 원문을 한국어로 옮겨 적는 것이 아닙니다. 자료를 확인한 뒤, 읽는 사람이 흐름을 따라갈 수 있게 다시 정리합니다. 낯선 말은 먼저 풀어 쓰고, 중요한 변화는 맥락을 붙이고, 표나 이미지가 필요한 부분은 따로 빼서 확인하기 쉽게 만듭니다. 사담이나 감상은 줄이고, 판단이 필요한 대목은 출처와 근거를 같이 둡니다.
...
- 주제 리서치: 공식 문서, 원문 자료, 최근 자료를 먼저 확인합니다.
- 출처 기반 정리: 중요한 주장 옆에 근거를 남기고, 애매한 내용은 애매하다고 표시합니다.
- 한국어 위키식 설명문 작성: 번역문처럼 한 문장씩 옮기지 않고, 배경과 흐름을 다시 엮습니다.
- 고정 목차가 있는 HTML 페이지 생성: 긴 글도 왼쪽 목차로 바로 이동할 수 있게 만듭니다.
- Chrome 스크린샷 이미지 첨부: 인물, 서비스, 논문, 공식 페이지처럼 눈으로 확인해야 하는 자료를 같이 둡니다.
- IYO Wiki 복붙용 `iyo-wiki.txt` 생성: HTML 결과를 위키 문법에 가까운 텍스트로 뽑아 줍니다.
- `research.md`, `qa.md` 기록: 어디를 봤고, 무엇을 확인했고, 무엇이 한계였는지 남깁니다.
- 선택적으로 Typst/PDF 소책자 출력: 화면용 HTML과 별개로 읽고 인쇄하기 좋은 문서판을 만듭니다.
...
이요 위키에 붙여 넣을 텍스트가 필요하면 최종 HTML을 만든 뒤 아래 명령을 실행합니다. 이 파일은 HTML을 대체하는 “새 결과물”이라기보다, 이미 정리된 흐름을 위키 편집창으로 옮기기 위한 중간 형태입니다.

```sh
node scripts/export-iyo-wiki-text.js outputs/YYYY-MM-DD-topic-slug
```

그러면 같은 폴더에 `iyo-wiki.txt`가 생깁니다. 제목, 문단, 링크, 목록, 표, 인용문, 이미지 캡션을 IYO Wiki에 붙여 넣기 쉬운 문법으로 바꿔 줍니다. 복붙 후에도 위키 안에서 문장을 줄이거나 이미지를 교체할 수 있게, 가능한 한 편집 가능한 텍스트로 남깁니다.

이미지는 `{{iyoimage ...}}` 형태의 자리표시자로 들어갑니다. 실제 위키에 올릴 때는 이미지를 업로드한 뒤 `file="..."` 경로를 맞춰 주면 됩니다.

## Typst/PDF 소책자

논문처럼 읽히는 소책자 PDF가 필요하면 아래 명령을 실행합니다. HTML은 화면에서 훑어보는 데 좋고, Typst/PDF는 저장하거나 공유하거나 인쇄해서 읽는 데 좋습니다. 그래서 같은 리서치를 두 번 쓰는 것이 아니라, 최종 HTML에서 문서용 판본을 뽑는 방식으로 봅니다.

```sh
node scripts/build-typst-booklet.js outputs/YYYY-MM-DD-topic-slug
```

그러면 `typst/main.typ`, `typst/iiki-booklet.typ`, `typst/index.pdf`가 만들어집니다. 기본 판형은 A5 소책자에 가깝고, 긴 리서치를 읽기 좋은 본문 폭과 여백으로 다시 배치합니다. Typst CLI가 없으면 `.typ` 파일만 만들 수 있고, PDF 컴파일은 나중에 하면 됩니다.
