...
- Desktop TOC: `position: fixed`, `top` set, no fixed `bottom`, `height: fit-content`, and equal top/bottom padding. Use `max-height` only as a fallback for very long TOCs.
